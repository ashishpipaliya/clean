...
class _SplashPageState extends State<SplashPage> {
  ...
  void initState() {
    super.initState();
    // Defer navigation until after first frame is rendered
  ...
  Future<void> _navigateToNextScreen() async {
    ...
    // Check authentication status in isolate/compute to avoid blocking UI
    final authRepository = getIt<AuthRepository>();
    final isAuthenticated = await authRepository.isAuthenticated();

    if (!mounted) return;

    // Use pushReplacement to avoid keeping splash in navigation stack
    if (isAuthenticated) {
      context.pushReplacement(DashboardPage.routeName);
    } else {
